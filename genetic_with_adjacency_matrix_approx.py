...
from jpso import find_vertices_to_correct_cover
...
def make_genome_feasible_approx(population, adjacency_matrix):
    feasible_solutions = []
    for _ in range(len(population)):
        A = deepcopy(adjacency_matrix)
        feasible_solutions.append(find_vertices_to_correct_cover(population[_], A))
...
def genetic_adjacency_matrix_approx(graph_data, pop_size, timeout):
    ...
    initial_population = make_genome_feasible_approx(initial_population, adjacency_matrix)
    population = []
    ...
    while time.time() - start_time <= timeout * 60:
        ...
        mutated = make_genome_feasible_approx(mutated, adjacency_matrix)
